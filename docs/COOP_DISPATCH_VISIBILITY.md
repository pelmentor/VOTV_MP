...
| kerfur conversion verbs (`dropKerfurProp`/`spawnKerfuro`) | **`EX_LocalVirtualFunction` ubergraph self-calls** (bytecode-measured 2026-07-13; the inner spawns EX_CallMath→Func-VISIBLE) | **NOW CAUGHT (observe-only) via the GNatives[0x45] swap** — the VM-dispatch substrate SHIPPED increment 1 `722fbe18` (`ue_wrap/vm_dispatch.cpp` + `coop/creatures/kerfur_form_assembler.cpp`); the verb entry + its in-bracket FinishSpawningActor/K2_DestroyActor are all captured. Containment [V] (static body walk + 18/18 hands-on). CAPTURE + **DRAIN**-at-birth + CONVERGE (2a CONVERGED on **FULL-UNIFY A+** 2026-07-14 impl /qf; the earlier REPOINT/eid-reuse was **MEASURED FALSE** — eid is per-form, K stable → drain-not-migrate, `BindFormActor` NOT split; the 4 converge crutches collapse to 1; observe pass COMPLETE + GREEN incl the loot reject gate) = 2a+, `[[project-vm-dispatch-2a-capture-2026-07-14]]` + `COOP_VM_DISPATCH_PLAN.md` §3 SUPERSEDED banner | interim (still live until 2a): express-edge first-refusal (turn_off, V take-10 log) + POLL as the client request driver (host turn-ON arm structurally dead) | [V opcode+containment; bytecode+runtime 2026-07-13] |
