...
| kerfur conversion verbs (`dropKerfurProp`/`spawnKerfuro`) | **`EX_LocalVirtualFunction` ubergraph self-calls** (bytecode-measured 2026-07-13; the inner spawns EX_CallMath→Func-VISIBLE) | **INVISIBLE today; PLANNED = VM verb brackets** (`COOP_VM_DISPATCH_PLAN.md` — the form-flip assembler, HALT-gated) | interim: express-edge first-refusal (turn_off, V take-10 log) + POLL as the client request driver (host turn-ON arm structurally dead — the destroy seam drains the element with the death) | [RD; opcode V bytecode 2026-07-13] |
