...
`research/findings/world-systems/votv-gamerules-settings-RE-2026-07-09.md`; class: `COOP_WORLD_PROP_DIVERGENCE.md`.
...
> `research/findings/kerfur/kerfur-identity-authority-and-module-refactor-DESIGN-2026-06-29.md` Part 3).** The
...
|---|---|---|
| `coop/element/identity.h`, `identity_create.cpp` (`CreateOrAdopt{Prop,Npc,WorldActor}Mirror` -- the ONE wire-mirror bind, sealed to `coop::element`), `identity_destroy.cpp` (`RetireMirror` -- the ONE type-dispatched destroy), `quiescence_drain.cpp` (the join-window ORDER owner -- the drain-edge reconcile SEQUENCE + the deferred queues; was `identity_reconcile.cpp` + `pile_reconcile.cpp` groups B+C, consolidated 2026-06-30; **+ the SPAWN-REVALIDATION queue 2026-07-11 `8a2b04d0`: every in-episode wire prop expression is captured and, at the drain (step 4, pre-destroys), re-expressed iff its row is still dead/absent -- the churn-victim-with-no-save-recreate class, see COOP_ENTITY_EXPRESSION_MAP "Join-window PROVISIONALITY". ORDER take-3 fix `2fefd161`: RunReconcile (joinSweep param retired) runs at the quiescence FIRE EDGE BEFORE the doom sweep, claims still armed -- doom judges LAST (the tail placement doomed-then-resurrected 230 props = the 2.5 fps storm); the one-shot orphan census moved to the sweep tail**) | THE keystone: bind / adopt / morph / reconcile / retire, for all 3 streamed kinds (Prop/Npc/WorldActor). `[V; spawn-revalidation + order fix AS-BUILT, take-4 hands-on pending]` | — |
...
- **Screens/panels gap list** (the rest of "sync every screen and panel"): reactor rods ->
  ...
  tasks roll -> serverBox RE. Shapes + boundaries: `research/findings/computers-devices/votv-screens-panels-sync-DESIGN-2026-07-03.md`
