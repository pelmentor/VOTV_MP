...
| `event_active_sync.cpp` | Join-during-event Phase 0 (2026-07-04 `d2d7eedb`): read-only host 1 Hz mirror of the NATIVE activeEvents registry (gamemode.activeEvents_senders membership diff) -> BEGIN/END edge log + join-edge would-be-EventSnapshot log. Seam PROVEN live (obelisk/alarm BEGIN+END, eventforce run 21:38; piramid BEGIN/END elapsed=211s, piramidforce run 23:19). Phase 1 adds the wire (COOP_EVENT_JOIN.md); per-event scenario docs = docs/events/ | none yet (Phase 0 log-only) `[V: matching real log]` |
| `piramid_sync.cpp` (creatures/) | Walking-pyramid event choreography lane (v97, 2026-07-04 late; docs/events/piramid.md): client mirror brain suppression (3 PRE-cancelled timer handlers, tick kept alive for beams/look-at) + host gather-commit relay + client native checkIfReached re-dispatch. Pose/spawn/despawn ride world_actor_sync (piramid2_C allowlisted; EX-catch enroll + pose-walk dead-retire added there); wisps ride the npc lane; 'piramid' verdict FLIPPED to no-replay | PyramidGather `[AS-BUILT; autonomous e2e PASS 23:19 -- relay + replay dist=9495 + destroy propagation; hands-on visual pending]` |
