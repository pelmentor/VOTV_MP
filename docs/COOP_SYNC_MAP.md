...
| `hand_item.cpp` (per-slot state store + owner change-poll + the display mirrors), `local_streams.cpp` (the owner poll site: an Aprop_C in `holding_actor` routes HERE, not to the prop pipeline), `event_feed.cpp` (router case), `subsystems.cpp` (connect replay), `ue_wrap/engine_attach.cpp` (GetActorRootComponent + SetActorRelativeLocation) | **v105 hotbar HAND-ITEM display axis** (RULE-1 root fix 2026-07-06): the item in a player's quick-slot hand is PLAYER EXPRESSION (MTA current-weapon shape) — owner announces `{class,name}` on change (~1 reliable msg/switch, empty debounced ~250 ms across updateHold's respawn flicker); peers keep a display-only mirror (physics/collision off, spawn/destroy echo-suppressed via prop_echo_suppress) attached to the puppet root at the native hold offset. Replaces the world-prop path whose host-side PRE-QUIESCENCE gate never lifted (hands-on 12:40:05: `no local match` x1809, host hand never mirrored) and whose release edge dropped physics litter per switch. Trash clump/pile carry (non-Aprop_C holding_actor) unchanged | HandItem (89) + connect replay `[AS-BUILT 2026-07-06, DLL E1C18158 4/4, smoke x2 PASS (axis silent at idle by design — smoke peers don't touch the hotbar); hands-on 0ab-HANDITEM pending]` |
