...
| `client_model.cpp` (name→asset + THE ApplySkinToBody), `skin_registry.cpp` (pak-folder catalog + the v94 BUILTIN kerfur-body table), `local_body.cpp` (LOCAL first-person body + the ini-persisted choice), `player_handshake.cpp` (per-slot skin state + the wire fields), `ui/skins_panel.cpp` (the F1 browser) | v93 player SKINS (docs/COOP_CLIENT_MODEL.md §3) | SkinChange (82) + skin fields inside Join/PlayerJoined `[AS-BUILT 2026-07-02]` |
