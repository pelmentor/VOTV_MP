...
| `firefly_sync.cpp` | Ambient fireflies (peer-symmetric; the shipped OWNER-EFFECT precedent — [[feedback-owner-effect-rule]]) | FireflySpawn `[V]` |
| `spawn_authority.cpp` | T1 Inc-1 (2026-07-10 `e6c1371b`, docs/COOP_RNG_AUTHORITY.md T1 STRUCTURAL DESIGN): client shared-world spawner suppression — t1 tick-park (insomniac+fossilhound spawners; 1 Hz first-instance hunt then 15 s reconcile; restore on session end) + t3 cancels (mushroomMaster/mushroomSpawner/pinecone/yellowWisp, absorbed from ambient_spawner_suppress) + shipping tripwire via npc_sync pass-through | none (suppression, no wire) `[AS-BUILT; VERIFY-gate passed by census: client tickEnabled=0 + zero post-park re-arms vs host control 26 re-arms]` |
