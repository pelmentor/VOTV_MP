...
| Kerfur (prop⇄NPC) | conversion verbs = **`EX_LocalVirtualFunction` ubergraph self-calls** (bytecode-measured 2026-07-13; the INNER spawns are EX_CallMath→**Func-VISIBLE** — "EX_CallMath is a wall" was FALSE, see `docs/COOP_VM_DISPATCH_PLAN.md` §0) | **OWNER (in progress): the form-flip ASSEMBLER on VM verb brackets** (`COOP_VM_DISPATCH_PLAN.md`). **Increment 1 SHIPPED `722fbe18` (observe-only)**: the GNatives[0x45] substrate catches both verbs; containment [V]-confirmed (static body walk — both verbs standalone synchronous, no latent node — + 18/18 hands-on: spawn + self-destroy both IN-bracket, both peers). The CAPTURE + **DRAIN**-at-birth + CONVERGE that retires everything below = 2a+ (2a CONVERGED on **FULL-UNIFY A+** 2026-07-14 impl /qf, superseding the 07-13 repoint design: the impl pass MEASURED the eid is per-form + K stable (`kerfur_convert.cpp:188-258`), so "migrate the eid onto B / split BindFormActor" was FALSE — A+ **DRAINS** A's prop-eid at birth (a subtraction, A dies husk), KerfurId re-key STAYS at converge, `BindFormActor` NOT split; the 4 converge crutches collapse to 1; observe pass COMPLETE + GREEN incl the loot reject gate. See `[[project-vm-dispatch-2a-capture-2026-07-14]]`). Interim as-built: turn_off = express-edge FIRST-REFUSAL `ded3f793` (**live-fired 2x clean, take-10 log 12:59:10-11, 0 cm**) + turn-on destroy-edge capture `873ce32a` **[BROKEN — never fired: the destroy seam is a POST hook; `GetActorLocation` on the destroyed actor reads (0,0,0) → the 5 m filter silently declined all; do NOT fix-forward beyond the optional 1h bridge]** + the death-watch POLL (client request driver; **host turn-ON arm structurally DEAD**: the seam drains the element with the death — the "element present + actor dead" premise never holds for a host prop) | host **KerfurId** (spans both forms) + the per-form eid | kerfur_entity, kerfur_convert, kerfur_command, kerfur_prop_adoption |
